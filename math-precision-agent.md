...
**Uncertainty/spread/confidence bands must account for self-inflicted truncation error, not just the upstream source's stated uncertainty.** When code computes both (a) a value via truncating division, and (b) a protective margin/spread/confidence-band meant to bound how wrong that value could be, verify the margin calculation actually includes the truncation remainder discarded in step (a) — not just an externally-supplied uncertainty figure (e.g. an oracle's own reported confidence). A price/rate normalization that floors `pU / divisor` and then derives its spread from `conf` alone, ignoring `pU % divisor`, is silently blind to a deterministic, code-introduced error source that can exceed the oracle's own stated uncertainty several times over for low-magnitude quotients. Concretely check: for the smallest plausible quotient in the supported input range (lowest-priced supported asset, most extreme decimal
exponent), compute `remainder / trueValue` in the same units as the spread — if that ratio is larger than the spread the code actually produces, the "protective band" is measuring the wrong thing and provides false confidence rather than real protection. This is especially dangerous because, unlike random oracle noise, the missing error is fully deterministic and reproducible by anyone who can read the same code — an attacker doesn't need to wait for a favorable price, they can compute the exploitable gap directly from the feed's exponent alone.
